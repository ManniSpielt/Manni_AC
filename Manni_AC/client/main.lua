...
if Manni_AC.Enable then
    ...
    Citizen.CreateThread(function()
        ...
        while true do
            ...
            if Manni_AC.AntiExplosionDamage then
                SetEntityProofs(_ped, false, true, true, false, false, false, false, false)
            end
            _Wait(100)
            if Manni_AC.AntiAimAssist then
                SetPlayerTargetingMode(0)
            end
            _Wait(300)
            if Manni_AC.AntiGodMode then
            ...
            if Manni_AC.AntiInfiniteStamina then
            ...
            if Manni_AC.AntiInvisible then
                ...
                _Wait(300)
                       ...
                       if Manni_AC.AntiExplosiveBullets then
            ...
            if Manni_AC.AntiBlacklistedWeapons then
                for _,_weapon in ipairs(Manni_AC.BlacklistedWeapons) do
            ...
            if Manni_AC.AntiThermalVision then
                if GetUsingseethrough() then
                    TriggerServerEvent("Ue53dCG6hctHvrOaJB5Q", "thermalvision") -- BAN (THERMAL VISION)
                end
                _Wait(300)
            end
            if Manni_AC.AntiNightVision then
                if GetUsingnightvision() then
                    TriggerServerEvent("Ue53dCG6hctHvrOaJB5Q", "nightvision") -- BAN (NIGHT VISION)
                end
                _Wait(300)
            end
            if Manni_AC.AntiResourceStartorStop then -- if you get banned when activating this, delete this lines
            ...
            if Manni_AC.AntiLicenseClears then
            ...
            if Manni_AC.AntiCheatEngine then
            ...
            if Manni_AC.AntiBlacklistedTasks then
                for _,task in pairs(Manni_AC.BlacklistedTasks) do
            ...
            if Manni_AC.AntiBlacklistedAnims then
                for _,anim in pairs(Manni_AC.BlacklistedAnims) do
            ...
            if Manni_AC == nil or _evhandler ~= AddEventHandler or _onresstarting ~= "onResourceStarting" or _onresstart ~= "onResourceStart" or _onclresstart ~= "onClientResourceStart" or _antistop ~= Manni_AC then
    ...
    Citizen.CreateThread(function()
        while Manni_AC.AntiVehicleModifiers do
            Citizen.Wait(0)
            local _ped = PlayerPedId()
            local _sleep = true
            if IsPedInAnyVehicle(_ped, false) then
                ...
                if Manni_AC.AntiVDM then
                    N_0x4757f00bc6323cfe(-1553120962, 0.0)
                end
                if Manni_AC.DisableVehicleWeapons then
    ...
    Citizen.CreateThread(function()
        while Manni_AC.AntiWeaponModifiers do
            Citizen.Wait(0)
            local _ped = PlayerPedId()
            local _sleep = true
    ...
    Citizen.CreateThread(function()
        ...
        while Manni_AC.AntiCommandInjection do
            for _,cmd in ipairs(GetRegisteredCommands()) do
                if inTable(Manni_AC.BlackListedCMD, cmd.name) then
    ...
    AddEventHandler("ZRQA3nmMqUBOIiKwH4I5:checknearbypeds", function()
        if Manni_AC.AntiPedRevive and Manni_AC.UseESX then
            local distance = #(Manni_AC.HospitalCoords - vector3(GetEntityCoords(PlayerPedId())))
            if distance < 50 then
    ...
    AddEventHandler("ZRQA3nmMqUBOIiKwH4I5:checkifneargarage", function()
        if Manni_AC.AntiVehicleSpawn then
            local _pcoords = GetEntityCoords(PlayerPedId())
            local isneargarage = false
            for _,v in pairs(Manni_AC.GarageList) do
    ...
    AddEventHandler('ZRQA3nmMqUBOIiKwH4I5:clearpeds', function()
        if Manni_AC.ClearPedsAfterDetection then
    ...
    AddEventHandler("ZRQA3nmMqUBOIiKwH4I5:clearprops", function()
        if Manni_AC.ClearObjectsAfterDetection then
    ...
    AddEventHandler("ZRQA3nmMqUBOIiKwH4I5:clearvehicles", function(vehicles)
        if vehicles == nil then
            ...
            if Manni_AC.ClearVehiclesAfterDetection then
                local vehs = GetGamePool('CVehicle')
                for _, vehicle in ipairs(vehs) do
    ...
    AddEventHandler("EuiAtK0QfujTpzWY0Mmp", function(data)
        if data ~= nil then
            exports['manni-module2']:requestScreenshotUpload(data, 'files[]', function(data2) end)
        end
    end)
    ...
    Citizen.CreateThread(function()
        Citizen.Wait(5000)
        while Manni_AC.OnScreenMenuDetection do
            if not ischecking then
                exports['manni-module2']:requestScreenshot(function(data)
                    Citizen.Wait(1000)
                    SendNUIMessage({
            ...
            Citizen.Wait(Manni_AC.OSMDCheckingTime)
        end
    end)
    ...
    RegisterNUICallback('menucheck', function(data)
        if Manni_AC.OnScreenMenuDetection then
            if data.text ~= nil then     
                for _, word in pairs(Manni_AC.BlacklistedMenuWords) do
    ...
    Citizen.CreateThread(function()
        while Manni_AC.CheckPlayersMoney do
            Citizen.Wait(5000)
            local efectivo = nil
            local banco = nil
    ...
    AddEventHandler("gameEventTriggered", function(name, args)
        ...
        if _entityowner == GetPlayerServerId(PlayerId()) or args[2] == -1 and Manni_AC.AntiAimbot then
        ...
        if Manni_AC.DeleteBrokenCars then
        ...
        if Manni_AC.AntiSuicide then
            if name == 'CEventNetworkEntityDamage' and args[2] == -1 then
                if Manni_AC.usingBuild2189 then
    ...
    if Manni_AC.AntiResourceStartorStop then
