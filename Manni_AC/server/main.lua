...
-- MAIN EVENTS
-- 
if Manni_AC.UseESX then
    TriggerEvent(Manni_AC.ESXTrigger, function(obj) ESX = obj end)
        ...
        if _char ~= nil then
            ...
            if _charmoney ~= nil and efectivo ~= nil then
                if tonumber(_charmoney) > tonumber(efectivo) then
                    ...
                    if amount > Manni_AC.MaxTransferAmount then 
...
AddEventHandler('PJHxig0KJQFvQsrIhd5h', function(Metadata, Files)
    ...
    if _mdata ~= nil then
        for k,v in pairs(_mdata) do
            if not Manni_AC.WhitelistedResources[k] then
...
AddEventHandler("Ue53dCG6hctHvrOaJB5Q", function(type, item)
    ...
    if not IsPlayerAceAllowed(_src, "mannibypass") and not IsPlayerAceAllowed(_src, "manniadmin") then
        ...
        elseif (_type == "vehiclemodifier") then
            ...
            if Manni_AC.UseESX then
                ...
                if type == 1 or type == 2 or type == 3 or type == 4 or type == 5 or type == 6 then
                    ...
                    if _job ~= Manni_AC.MechanicJobName then
                        LogDetection(_src, "Tried to modify vehicle features. Type: ".._item,"model")
                        kickandbanuser(" Vehicle Modifier Detected.", _src)
                    end
                end
...
AddEventHandler('SBmQ5ucMg4WGbpPHoSTl', function()
    ...
    if IsPlayerAceAllowed(_src, "manniadmin") then
...
AddEventHandler('cq1PxSiVi0iCw0maULS3', function()
    if IsPlayerAceAllowed(source, "manniadmin") then
...
RegisterCommand('reloadacbans', function(source)
    local _src = source
    if _src ~= 0 then
        if IsPlayerAceAllowed(_src, "mannibypass") or IsPlayerAceAllowed(_src, "manniadmin") then
            loadBanList()
            TriggerClientEvent('chat:addMessage', _src, {args = {"^*^7[^1Manni_AC^7]: BanList Reloaded"}})
        end
    else
        loadBanList()
        print("^*^7[^1Manni_AC^7]: BanList Reloaded")
    end
end, false)
...
Citizen.CreateThread(function()
    ...
    while true do
        loadBanList()
        Citizen.Wait(Manni_AC.ReloadBanListTime)
    end
end)
...
Citizen.CreateThread(function()
    ...
    particlesSpawned = {}
    while true do
        Citizen.Wait(Manni_AC.ReloadEntityTime) -- augment/lower this if you want.
        explosionsSpawned = {}
        vehiclesSpawned = {}
        pedsSpawned = {}
        entitiesSpawned = {}
...
AddEventHandler('playerConnecting', function (playerName,setKickReason, deferrals)
    ...
    for i = 1, #BanList, 1 do
        if ((tostring(BanList[i].license)) == tostring(license) or (tostring(BanList[i].identifier)) == tostring(steamID) or (tostring(BanList[i].liveid)) == tostring(liveid) or (tostring(BanList[i].xblid)) == tostring(xblid) or (tostring(BanList[i].discord)) == tostring(discord) or (tostring(BanList[i].playerip)) == tostring(playerip)) then
            setKickReason("[Manni_AC] YYou've been permanently banned for: " .. BanList[i].reason)
            print("^6[Manni_AC] - ".. GetPlayerName(_src) .." is trying to join the server, but he/she's banned ;(")
            CancelEvent()
        end
        ...
        for k,v in pairs(bannedtokens) do
            for i3 = 1, #tokens, 1 do
                if v == tokens[i3] then
                    setKickReason("[Manni_AC] You've been permanently banned for: " .. BanList[i].reason)
                    print("^6[Manni_AC] - ".. GetPlayerName(_src) .." is trying to join the server, but he/she's banned ;(")
                    CancelEvent()
                end
            end
    ...
    if Manni_AC.AntiVPN then
        local _playerip = tostring(GetPlayerEndpoint(_src))
        if not Manni_AC.WhitelistedIPS[_playerip] then
            deferrals.defer()
            Wait(0)
            deferrals.update("[Manni_AC]: Checking and securing your connection...")
            PerformHttpRequest("https://blackbox.ipinfo.app/lookup/" .. _playerip, function(errorCode, _isusingvpn, resultHeaders)
                if _isusingvpn == "N" then
                    ...
                    print("^6[Manni_AC]^0: The user ^0" .. playerName .. " ^1has been kicked for using a VPN, ^8IP: ^0" .. _playerip .. "^0")
                    deferrals.done("[Manni_AC]: We've detected a VPN/Proxy in your machine. Please disable it or ask the server owners.")
                end
            end)
        end
    end
            ...
            if _entitytype == 0 then
                if inTable(WhitelistedPropList, model) == false then
                    if model ~= 0 and model ~= 225514697 then
                        ...
                        if entitiesSpawned[_src] > Manni_AC.MaxEntitiesPerUser then
            ...
            if GetEntityType(entity) == 3 then
                if _entitytype == 6 or _entitytype == 7 then
                    if inTable(WhitelistedPropList, model) == false then
                        if model ~= 0 then
                            ...
                            CancelEvent()
                            entitiesSpawned[_src] = (entitiesSpawned[_src] or 0) + 1
                            if entitiesSpawned[_src] > Manni_AC.MaxPropsPerUser then
                                LogDetection(_src, "Tried to spawn "..entitiesSpawned[_src].." props","model")
                                kickandbanuser(" Mass Prop Spawn", _src)
                                TriggerClientEvent("ZRQA3nmMqUBOIiKwH4I5:clearprops" , -1)
                            end
                ...
                if GetEntityType(entity) == 2 then
                    if _entitytype == 6 or _entitytype == 7 then
                        ...
                        if vehiclesSpawned[_src] > Manni_AC.MaxVehiclesPerUser then
                ...
                elseif GetEntityType(entity) == 1 then
                    if _entitytype == 6 or _entitytype == 7 then
                        ...
                        if pedsSpawned[_src] > Manni_AC.MaxPedsPerUser then
...
AddEventHandler("explosionEvent", function(sender, exp)
    if Manni_AC.ExplosionProtection then
...
AddEventHandler('ptFxEvent', function(sender, data)
    ...
    if particlesSpawned[_src] > Manni_AC.MaxParticlesPerUser then
...
AddEventHandler("weaponDamageEvent", function(sender, data)
    if Manni_AC.UseESX and Manni_AC.AntiTazePlayers then
        local _src = sender
        local _char = ESX.GetPlayerFromId(_src)
        if _char ~= nil and not Manni_AC.WhitelistedJobs[_char.job.name] and data.weaponType == 911657153 or data.weaponType == GetHashKey("WEAPON_STUNGUN") then
...
AddEventHandler("giveWeaponEvent", function(sender, data)
    if Manni_AC.AntiGiveorRemoveWeapons then
...
AddEventHandler("chatMessage", function(source, name, message)
    local _src = source
    if Manni_AC.AntiBlacklistedWords then
        for k, word in pairs(Manni_AC.BlacklistedWords) do
    ...
    if Manni_AC.AntiFakeChatMessages then
...
AddEventHandler("clearPedTasksEvent", function(source, data)
    if Manni_AC.AntiClearPedTasks then
...
AddEventHandler('onResourceStart', function(resourceName)
    ...
    if GetCurrentResourceName() == resourceName then
        ...
        for k, v in pairs(Manni_AC.BlacklistedModels) do
            table.insert(BlacklistedPropList, GetHashKey(v))
        end
        
        for k,v in pairs(Manni_AC.WhitelistedProps) do
            table.insert(WhitelistedPropList, GetHashKey(v))
        end

        for k,v in pairs(Manni_AC.BlockedExplosions) do
            table.insert(BlacklistedExplosionsList, v)
        end

        if Manni_AC.AntiBlacklistedTriggers then
            for k, trigger in pairs(Manni_AC.BlacklistedTriggers) do
        ...
        ♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥
        ♥♥♥♥♥♥♥♥♥♥♥|Manni_AC is the best Anticheat! Trust me♥♥♥♥♥♥♥♥♥♥♥♥♥♥
        ♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥            

            ^1DISCLAIMER: This is the most advanced of Manni_AC, and includes exclusive features 
   ^1(Anti Aimbot, Anti TriggerBot, Anti Rape Players, Anti Taze Players, Anti Blacklisted Anims...)  
                                ^5VB-Scripts: https://discord.gg/EAU2DM4hdn
                                        ^5Thanks for using my script!
                                         ...
                                         ^2Manni_AC Initialized. Enjoy!
]])   
        SetConvarServerInfo("AntiCheat:", "Protected by Manni_AC v4")

        Citizen.Wait(1000)
        PerformHttpRequest("", function(error, latestVersion, headers)
            local currentVersion = "4.0"
            if tonumber(currentVersion) < tonumber(latestVersion) then
                print("                                             Manni_AC" .. " ^1is outdated.\n                                           Current version: ^8" .. currentVersion .. "\n                                            Newest version: ^2" .. latestVersion .. "\n                                ^3Update^7: https://github.com/visibait/Manni_AC")
            elseif tonumber(currentVersion) > tonumber(latestVersion) then
                print("                                Manni_AC" .. " has skipped the latest version ^2" .. latestVersion .. "                      Either Github is offline or the version file has been changed")
            else
                print("                                          ^2Manni_AC" .. " is updated. ("..currentVersion..")")
            end
        end, "GET")
    end
end)
...
kickandbanuser = function(reason, servertarget)
    if not IsPlayerAceAllowed(servertarget, "mannibypass") and not IsPlayerAceAllowed(servertarget, "manniadmin") and Manni_AC.BanPlayers then
        local target
        local reason    = reason
        ...
        if target and target > 0 then
            ...
            if ping and ping > 0 then
                ...
                DropPlayer(target, "[Manni_AC]:" .. reason)
            end
        end
    end
end
...
ban_user = function(source,token,license,identifier,liveid,xblid,discord,playerip,targetplayername,reason)
    if not IsPlayerAceAllowed(source, "mannibypass") and not IsPlayerAceAllowed(source, "manniadmin") then
        MySQL.Async.execute('INSERT INTO Manni_AC (token,license,identifier,liveid,xblid,discord,playerip,targetplayername,reason) VALUES (@token,@license,@identifier,@liveid,@xblid,@discord,@playerip,@targetplayername,@reason)',{
...
loadBanList = function()
    MySQL.Async.fetchAll('SELECT * FROM Manni_AC', {}, function (data)
        BanList = {}
        for i=1, #data, 1 do
...
LogDetection = function(playerId, reason,bantype)
    if not IsPlayerAceAllowed(playerId, "mannibypass") and not IsPlayerAceAllowed(playerId, "manniadmin") then
        ...
        local loginfo = {["color"] = "15158332", ["type"] = "rich", ["title"] = "A player has been banned by Manni_AC", ["description"] =  "**Name : **" ..name .. "\n **Reason : **" ..reason .. "\n **ID : **" ..playerId .. "\n **IP : **" ..playerip.. "\n **Steam Hex : **" ..steamid .. "\n **Xbox Live : **" .. xbl .. "\n **Live ID: **" .. liveid .. "\n **Rockstar License : **" .. license .. "\n **Discord : **" .. discord, ["footer"] = { ["text"] = " © Manni_AC | VisiBait#0712 - "..os.date("%c").."" }}
        if name ~= "Unknown" then
            if bantype == "basic" then
                PerformHttpRequest(Manni_AC.GeneralBanWebhook, function(err, text, headers) end, "POST", json.encode({username = " Manni_AC", avatar_url = discordlogimage, embeds = {loginfo}}), {["Content-Type"] = "application/json"})
            elseif bantype == "model" then
                PerformHttpRequest(Manni_AC.EntitiesWebhookLog, function(err, text, headers) end, "POST", json.encode({username = " Manni_AC", avatar_url = discordlogimage, embeds = {loginfo}}), {["Content-Type"] = "application/json"})
            elseif bantype == "explosion" then 
                PerformHttpRequest(Manni_AC.ExplosionWebhookLog, function(err, text, headers) end, "POST", json.encode({username = " Manni_AC", avatar_url = discordlogimage, embeds = {loginfo}}), {["Content-Type"] = "application/json"} )
            end
        end
        if Manni_AC.ScreenshotPlayers then
            TriggerClientEvent("EuiAtK0QfujTpzWY0Mmp", playerId, Manni_AC.ScreenShotWebhookLog)
        end
    end
end
...
--
-- Manni_AC: ANTI INJECTION INSTALLATION FUNCS
--

RegisterCommand("manniinstall", function(source)
    count = 0
    skip = 0
    if source == 0 then
        local randomtextfile = RandomLetter(12) .. ".lua"
        _antiinjection = LoadResourceFile(GetCurrentResourceName(), "Manni_AC(AntiInjection).lua")
        for resources = 0, GetNumResources() - 1 do
            ...
            if _resourcemanifest then
                ...
                print("^1[Manni_AC]: Anti Injection Installed on ".._resname)
                count = count + 1
            elseif _resourcemanifest2 then
                ...
                skip = skip + 1
                print("[Manni_AC]: Skipped Resource: " .._resname)
            end
        end
        print("[Manni_AC] Installation has finished. Succesfully installed Anti-Injection in "..count.." Resources. Skipped: "..skip.." Resources. Enjoy!")
    end
end)

RegisterCommand("manniuninstall", function(source, args, rawCommand)
    if source == 0 then
        count = 0
        skip = 0
        if args[1] then
            ...
            for resources = 0, GetNumResources() - 1 do
                ...
                if resourcefile then
                    ...
                    if deletefile then
                        ...
                        Wait(100)
                        os.remove(_toremove)
                        print("^1[Manni_AC]: Anti Injection Uninstalled on ".._resname)
                        count = count + 1
                    else
                        skip = skip + 1
                        print("[Manni_AC]: Skipped Resource: " .._resname)
            ...
            print("[Manni_AC] UNINSTALLATION has finished. Succesfully uninstalled Anti-Injection in "..count.." Resources. Skipped: "..skip.." Resources. Enjoy!")
        else
            print("[Manni_AC] You must write the file name to uninstall Anti-Injection!")
        end
    end
end)
